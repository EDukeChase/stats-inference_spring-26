...
::: {style="font-size: 200%;"}
The discrete case and the binomial distribution
:::
...
::: {style="font-size: 200%;"}
The Continuous Case and the Gamma Distribution
:::
...
::: {style="font-size: 200%;"}
The distribution of a minimum by example
:::
...
::: {style="font-size: 200%;"}
The general distribution of minimums and maximums
:::
...
::: {style="font-size: 200%;"}
The expectation of a function $X$
:::
...
::: {style="font-size: 200%;"}
Finding mgfs and the Poisson distribution
:::
...
::: {style="font-size: 200%;"}
The mgf uniquely determines a distribution
:::
...
For an i.i.d. sample of continuous random variables $X_1, X_2, \ldots, X_n$ with cdf $F$, the joint pdf of $(X_{(1)}, X_{(n)})$ is
$$
f_{X_{(1)}, X_{(n)}}(x, y) = n(n-1)[[F(y) - F(x)]^{n-2}f(x)f(y),
$$
for $x<y$.

## Proof of min/max pdf

## Example 1.6.1

Determine the joint pdf of $(X_{(1)}, X_{(15)})$ when $X_1, X_2, \ldots, X_{15} \stackrel{i.i.d.}{\sim}\text{Uniform}(0, 1)$.
...
::: {style="font-size: 200%;"}
The distribution of all order statistics
:::

## PDF of all order statistics

For an i.i.d. sample of continuous random variables $X_1, X_2, \ldots, X_n$ with pdf $f$, the pdf of all $n$ order statistics is
\begin{align}
&f_{X_{(1)}, X_{(2)}, \ldots, X_{(n)}}(x_1, x_2, \ldots, x_n)\\
&= n!f(x_1)f(x_2)\cdots f(x_n)I(x_1<x_2<\cdots<x_{n-1}<x_n).
\end{align}
...
::: {style="font-size: 200%;"}
The joint distribution of $X_{(i)}$ and $X_{(j)}$
:::
